...
def processLinkBase(
    root: etree._Element, base: str, ns: str, params: dict, handlerPrefix: str
) -> int:
...
def processExtendedLink(
    element: etree._Element, base: str, ns: str, params: dict, handlerPrefix: str
) -> int:
    params["xlinkCount"] += 1
    localLocCount = 0
    xlink = {
        XLINK_ROLE: element.attrib.get(XLINK_ROLE),
    ...
    for node in element:
        ...
        if node_type == "locator":
            ...
            logging.error(
                "Unknown type found in xlink " + str(node_type) + " in " + str(base)
            )
...
def process_resource(
    ...
    output.write(processAttribute(resource, XLINK_ROLE, attr_type=None, params=params))
    # output.write(processAttribute(resource, XLINK_LABEL,
    #                               attr_type=str, params=params))
    output.write(processAttribute(resource, XML_LANG, attr_type=str, params=params))
    output.write(processAttribute(resource, AS, attr_type=None, params=params))

    # arc_to boolean attributes
    output.write(processAttribute(resource, ABSTRACT, attr_type=bool, params=params))
    output.write(processAttribute(resource, MERGE, attr_type=bool, params=params))
    output.write(processAttribute(resource, NILS, attr_type=bool, params=params))
    output.write(processAttribute(resource, STRICT, attr_type=bool, params=params))
    output.write(
        processAttribute(resource, IMPLICITFILTERING, attr_type=bool, params=params)
    )
    output.write(processAttribute(resource, MATCHES, attr_type=bool, params=params))
    output.write(processAttribute(resource, MATCHANY, attr_type=bool, params=params))
    output.write(
        processAttribute(resource, BINDASSEQUENCE, attr_type=bool, params=params)
    )
    ...
    output.write(
        processAttribute(resource, FALLBACKVALUE, attr_type=str, params=params)
    )
    ...
    output.write(
        processAttribute(resource, PARENTCHILDORDER, attr_type=str, params=params)
    )
    ...
    if resource_text and (resource_text) != "\n      ":
        ...
        resource_text = resource_text.replace('"', "'").replace("\\", "\\\\")
        if lang is not None:
    ...
    for arc in xlink["arcs"]:
        ...
        for arc_from in arc["fromloc"]:
            ...
            for arc_to in arc["toloc"]:
                ...
                triple_subject = getTurtleName(
                    arc_from, base, ns, params, handlerPrefix
                )
                triple_predicate = genRoleName(arc[XLINK_ARCROLE], 1, params)
                triple_object = getTurtleName(arc_to, base, ns, params, handlerPrefix)
                ...
                output.write(
                    processAttribute(
                        arc, XBRLDT_CONTEXTELEMENT, attr_type=str, params=params
                    )
                )
                output.write(
                    processAttribute(
                        arc, XBRLDT_TARGETROLE, attr_type=None, params=params
                    )
                )
                output.write(
                    processAttribute(arc, XBRLDT_CLOSED, attr_type=bool, params=params)
                )
                output.write(
                    processAttribute(arc, XBRLDT_USABLE, attr_type=bool, params=params)
                )
                ...
                output.write(
                    processAttribute(arc, COMPLEMENT, attr_type=bool, params=params)
                )
                output.write(processAttribute(arc, NAME, attr_type=str, params=params))
                output.write(
                    processAttribute(arc, PREFERRED_LABEL, attr_type=str, params=params)
                )
                # end of addition to xbrlimport / Raggett

                output.write(processAttribute(arc, USE, attr_type=str, params=params))
                output.write(
                    processAttribute(arc, PRIORITY, attr_type=int, params=params)
                )
                output.write(
                    processAttribute(arc, ORDER, attr_type=float, params=params)
                )
                output.write(
                    processAttribute(arc, WEIGHT, attr_type=float, params=params)
                )
                ...
                if arc_to.get(XLINK_TYPE, None) == "resource":
                    ...
                    if resource_text and (resource_text != "\n      "):
                        triple_object = genResourceName(params, handlerPrefix)
                output.write("    xl:to " + triple_object + " ;\n")
                output.write("    ] .\n\n")

                if arc_from.get(XLINK_TYPE, None) == "resource":
                    process_resource(
                        triple_subject, arc_from, base, ns, params, handlerPrefix
                    )

                if arc_to.get(XLINK_TYPE, None) == "resource":
                    process_resource(
                        triple_object, arc_to, base, ns, params, handlerPrefix
                    )
        ...
        for arc_from in arc["fromloc"]:
            for arc_to in arc["toloc"]:
                ...
                if found:
                    ...
                    output.write(
                        processAttribute(arc, XLINK_ROLE, attr_type=str, params=params)
                    )
                    output.write(
                        processAttribute(arc, USE, attr_type=str, params=params)
                    )
                    output.write(
                        processAttribute(arc, PRIORITY, attr_type=int, params=params)
                    )
                    ...
                    output.write(
                        processAttribute(
                            arc, XBRLDT_CLOSED, attr_type=bool, params=params
                        )
                    )
                    output.write(
                        processAttribute(
                            arc, XBRLDT_USABLE, attr_type=bool, params=params
                        )
                    )
                    output.write(
                        processAttribute(arc, COVER, attr_type=str, params=params)
                    )
                    output.write(
                        processAttribute(arc, AXIS, attr_type=str, params=params)
                    )
                    output.write(
                        processAttribute(arc, COMPLEMENT, attr_type=bool, params=params)
                    )
                    output.write(
                        processAttribute(arc, NAME, attr_type=str, params=params)
                    )
                    output.write("    .\n")
                output.write("\n")
...
def genLinkName(params: dict, handlerPrefix: str) -> str:
    params["linkCount"] += 1
    name = handlerPrefix + ":link" + str(params["linkCount"])
    return name


def genResourceName(params: dict, handlerPrefix: str) -> str:
...
def getTurtleName(
    loc: str, base: str, ns: str, params: dict, handlerPrefix: str
) -> str:
    ...
    if prefix == "_" and namespace is not None:
        ...
        uncased_namespaces = dict(
            (k.lower(), v) for k, v in params["namespaces"].items()
        )

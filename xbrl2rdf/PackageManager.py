...
class Taxonomies:
    ...
    def rebuildRemappings(self):
        ...
        for i, _remap in enumerate(remapOverlapUrls):
            ...
            for j in range(i - 1, -1, -1):
                ...
                if (
                    _packageURL != _packageURL2
                    and _prefix
                    and _prefix2
                    and (_prefix.startswith(_prefix2) or _prefix2.startswith(_prefix))
                ):
                    ...
                    if _prefix != _prefix2:
                        print(
                            "Packages overlap rewrite start strings "
                            + _prefix
                            + " and "
                            + _prefix2
                        )
    ...
    def save(self):
        with io.open(self.jsonFile, "wt", encoding="utf-8") as f:
            jsonStr = json.dumps(
                self.orderedPackagesConfig(), ensure_ascii=False, indent=2
            )
            f.write(jsonStr)
...
def xmlLang(element):
    return (element.xpath("@xml:lang") + element.xpath("ancestor::*/@xml:lang") + [""])[
        0
    ]
...
def parsePackage(filesource, metadataFile, fileBase, errors=[]):
    ...
    _file = filesource.file(metadataFile)[
        0
    ]  # URL in zip, plain file in file system or web
    try:
        tree = etree.parse(_file)
    except etree.XMLSyntaxError as err:
        addToLog(
            _("Package catalog syntax error %(error)s"),
    ...
    if ns in txmyPkgNSes:  # package file
        ...
        for eltName in ("name", "description"):
            ...
            if (
                not closest and eltName == "name"
            ):  # assign default name when none in taxonomy package
        ...
        for m in root.iterchildren(tag=nsPrefix + "versioningReports"):
            pkg["versioningReports"] = [
                r.get("href") for r in m.iterchildren(tag=nsPrefix + "versioningReport")
            ]
            ...
            for eltName in ("name", "description", "publisher"):
                ...
                for lang, elts in langElts.items():
                    if not lang:
                        addToLog(
                            _(
                                "Multi-lingual element %(element)s has no in-scope xml:lang attribute"
                            ),
                        ...
                        addToLog(
                            _(
                                "Multi-lingual element %(element)s has multiple (%(count)s) in-scope xml:lang %(lang)s elements"
                            ),
                            messageArgs={
                                "element": eltName,
                                "lang": lang,
        ...
        fileName = getattr(
            ...
            getattr(metadataFile, "name", metadataFile),  # for io.file named objects
        )  # for string
        ...
        for m in rewriteTree.iter(tag=tag):
            ...
            if prefixValue and replaceValue is not None:
                if prefixValue not in remappings:
                    ...
                    if replaceValue.endswith(os.sep) and not _normedValue.endswith(
                        os.sep
                    ):
                        _normedValue += os.sep
    ...
    for entryPointSpec in tree.iter(tag=nsPrefix + "entryPoint"):
        ...
        if not name:
            name = "<unnamed {0}>".format(unNamedCounter)
            unNamedCounter += 1
        ...
        for epDoc in entryPointSpec.iterchildren(nsPrefix + "entryPointDocument"):
            ...
            if longestPrefix:
                remappedUrl = _remappedUrl.replace(
                    os.sep, "/"
                )  # always used as FileSource select
...
def packageNamesWithNewerFileDates():
    ...
    for package in packagesConfig["packages"]:
        try:
            if package["fileDate"] < time.strftime(
                "%Y-%m-%dT%H:%M:%S UTC",
                time.gmtime(os.path.getmtime(freshenedFilename)),
            ):
    ...
    if packageFilename:
        ...
        try:
            ...
            if filesource.isZip:
                ...
                if not _dir:
                    raise IOError(
                        _("Unable to open taxonomy package: {0}.").format(
                            packageFilename
                        )
                    )
                ...
                if len(topLevelDirectories) != 1:
                    addToLog(
                        _(
                            "Taxonomy package contains %(count)s top level directories:  %(topLevelDirectories)s"
                        ),
                        messageArgs={
                            "count": len(topLevelDirectories),
                            "topLevelDirectories": ", ".join(
                                sorted(topLevelDirectories)
                            ),
                        },
                    ...
                    addToLog(
                        _("Taxonomy package does not contain a subdirectory META-INF"),
                    ...
                    packageFiles = [
                        f for f in _dir if f.endswith("/META-INF/taxonomyPackage.xml")
                    ]
                else:
                    addToLog(
                        _(
                            "Taxonomy package does not contain a metadata file */META-INF/taxonomyPackage.xml"
                        ),
                ...
                if len(packageFiles) < 1:
                    raise IOError(
                        _("Taxonomy package contained no metadata file: {0}.").format(
                            ", ".join(packageFiles)
                        )
                    )
                # if current package files found, remove any nonconforming package files
                if any(pf.startswith("_metaInf") for pf in packageFiles) and any(
                    not pf.startswith(_metaInf) for pf in packageFiles
                ):
                    packageFiles = [
                        pf for pf in packageFiles if pf.startswith(_metaInf)
                    ]
                elif any(pf.startswith("META-INF/") for pf in packageFiles) and any(
                    not pf.startswith("META-INF/") for pf in packageFiles
                ):
                    packageFiles = [
                ...
                addToLog(
                    _("Taxonomy package is not a zip file."),
                ...
                if os.path.basename(
                    filesource.url
                ) in TAXONOMY_PACKAGE_FILE_NAMES or (  # individual manifest file
                    os.path.basename(filesource.url) == "taxonomyPackage.xml"
                    and os.path.basename(os.path.dirname(filesource.url)) == "META-INF"
                ):
                    ...
                    raise IOError(
                        _(
                            "File must be a taxonomy package (zip file), catalog file, or manifest (): {0}."
                        ).format(
                            packageFilename, ", ".join(TAXONOMY_PACKAGE_FILE_NAMES)
                        )
                    )
            remappings = {}
            packageNames = []
            ...
            for packageFileUrl, packageFilePrefix, packageFile in packages:
                parsedPackage = parsePackage(
                    filesource, packageFileUrl, packageFilePrefix, errors
                )
            ...
            package = {
                ...
                "supersededTaxonomyPackages": parsedPackage.get(
...
def isMappedUrl(url):
    return packagesConfig is not None and any(
        url.startswith(mapFrom)

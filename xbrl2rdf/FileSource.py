...
archivePathSeparators = (
    ...
    ".xfd" + os.sep,
    ".frm" + os.sep,
    ".taxonomyPackage.xml" + os.sep,
) + (
    (".zip/", ".tar.gz/", ".eis/", ".xml/", ".xfd/", ".frm/", ".taxonomyPackage.xml/")
    if os.sep != "/"
    else ()
)  # acomodate windows and http styles
...
def archiveFilenameParts(filename):
    ...
    for archiveSep in archivePathSeparators:
        if filename and archiveSep in filename and (not archiveSep.startswith(".xml")):
            filenameParts = filename.partition(archiveSep)
            fileDir = filenameParts[0] + archiveSep[:-1]
            if isHttpUrl(fileDir) or os.path.isfile(
                fileDir
...
class FileNamedStringIO(
    io.StringIO
):  # provide string IO in memory but behave as a fileName string
...
class FileNamedBytesIO(
    io.BytesIO
):  # provide Bytes IO in memory but behave as a fileName string
    def __init__(self, fileName, *args, **kwargs):
...
class ArchiveFileIOError(IOError):
    def __init__(self, fileSource, errno, fileName):
        super(ArchiveFileIOError, self).__init__(
            errno, _("Archive {}").format(fileSource.url), fileName
        )
        self.fileName = fileName
...
class FileSource:
    def __init__(self, url):
        ...
        if (
            not (self.isZip or self.isEis or self.isXfd or self.isRss)
            and self.type == ".xml"
        ):
    ...
    def open(self, reloadCache=False):
        if not self.isOpen:
            ...
            if self.isZip:
                try:
                    self.fs = zipfile.ZipFile(
                        openFileStream(self.basefile, "rb"), mode="r"
                    )
                    self.isOpen = True
    ...
    def loadTaxonomyPackageMappings(self):
        if not self.mappedPaths and self.taxonomyPackageMetadataFiles:
            ...
            taxonomyPackage = PackageManager.parsePackage(
                self, metadata, os.sep.join(os.path.split(metadata)[:-1]) + os.sep
            )
    ...
    def isArchive(self):
        return (
            ...
            or self.isEis
            or self.isXfd
            or self.isInstalledTaxonomyPackage
        )

    @property
    def isTaxonomyPackage(self):
        return (
            self.isZip and self.taxonomyPackageMetadataFiles
        ) or self.isInstalledTaxonomyPackage
    ...
    def taxonomyPackageMetadataFiles(self):
        for f in self.dir or []:
            ...
            ):  # must be in a sub directory in the zip
                return [f]  # standard package
        return [
            f
            for f in (self.dir or [])
            if os.path.split(f)[-1] in TAXONOMY_PACKAGE_FILE_NAMES
        ]
    ...
    def fileSourceContainingFilepath(self, filepath):
        ...
        if referencedFileParts is not None:
            ...
            if referencedArchiveFile in self.referencedFileSources:
                ...
                referencedArchiveFile != self.basefile
                and referencedArchiveFile != self.baseurl
            ):
                referencedFileSource = openFileSource(filepath)
                if referencedFileSource:
                    self.referencedFileSources[
                        referencedArchiveFile
                    ] = referencedFileSource
        return None
        ...
        if archiveFileSource is not None:
            ...
            if archiveFileSource.isZip:
                try:
                    ...
                    return (
                        FileNamedTextIOWrapper(
                            filepath, io.BytesIO(b), encoding=encoding
                        ),
                        encoding,
                    )
                except KeyError:
    ...
    def exists(self, filepath):
        ...
        if archiveFileSource is not None:
            if filepath.startswith(archiveFileSource.basefile):
                ...
                archiveFileName = filepath[len(archiveFileSource.baseurl) + 1 :]
            if (
                archiveFileSource.isZip
                or archiveFileSource.isTarGz
                or archiveFileSource.isEis
                or archiveFileSource.isXfd
                or archiveFileSource.isRss
                or self.isInstalledTaxonomyPackage
            ):
...
def openFileStream(filepath, mode="r", encoding=None):
    ...
    if archiveFilenameParts(filepath):  # file is in an archive
        return openFileSource(filepath).file(
            filepath, binary="b" in mode, encoding=encoding
        )[0]
